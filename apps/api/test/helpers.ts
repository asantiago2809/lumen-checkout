...
export class MemoryStore implements CheckoutStore {
  ...
  async products() {
    return [...this.records.entries()]
      ...
      .map(([, value]) => structuredClone(value) as Stored<Product>);
  }
  async pending() {
    return [...this.records.entries()]
      .filter(
        ([key, value]) =>
          key.startsWith("TX#") && value.value.status === "PENDING",
      )
      .map(([, value]) => structuredClone(value) as Stored<Transaction>);
  }
  async commit(writes: Write[]) {
    for (const write of writes)
      if (
        (this.records.get(write.key)?.version ?? null) !== write.expectedVersion
      )
        throw new WriteConflict();
    for (const write of writes) {
      if (write.value === null) this.records.delete(write.key);
      else
        this.records.set(write.key, {
...
export class FakeGateway implements PaymentGateway {
  ...
  create = jest.fn(
    async (
      ...
      this.last = tx;
      return ok(this.provider(tx));
    },
  );
  get = jest.fn(async (_id: string): Promise<Result<ProviderTransaction>> =>
    ok(this.provider(this.last!)),
  );
  configured() {
    return this.enabled;
  }
  config = jest.fn(async () =>
    ok({
      environment: "sandbox" as const,
      paymentApiUrl: "https://sandbox.wompi.co/v1",
      publicKey: "public-placeholder",
      currency: "COP" as const,
      baseFeeInCents: 250000,
      deliveryFeeInCents: 1200000,
      acceptance: {
        terms: { token: "terms-token", url: "https://example.com/terms" },
        personalData: {
          token: "privacy-token",
  ...
  provider(tx: Transaction): ProviderTransaction {
    return {
      id: "provider-id",
      reference: tx.reference,
      status: this.status,
      amountInCents: tx.amounts.totalInCents,
      currency: "COP",
...
export const payment: PayInput = {
  cardToken: "token_ephemeral",
  installments: 1,
  acceptanceToken: "terms",
  acceptPersonalAuth: "privacy",
};
