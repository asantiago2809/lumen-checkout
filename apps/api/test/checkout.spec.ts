...
describe("checkout business invariants", () => {
  ...
  it("seeds once, recovers a session and never exposes owner or internal stock counters", async () => {
    ...
    expect(
      service.sessionView(value(await service.sessionFromToken(token))),
    ).not.toHaveProperty("id");
    const product = value(await service.product(input.productId));
    ...
    expect(await service.sessionFromToken("z".repeat(43))).toMatchObject({
      ok: false,
    });
    advance(86400001);
    expect(await service.sessionFromToken(token)).toMatchObject({ ok: false });
  ...
  it("persists incomplete non-card drafts, authorizes and clears them", async () => {
    ...
    const draft = {
      ...
      customer: { email: "unfinished@" },
      delivery: {},
    };
    expect(await service.saveDraft(owner, draft)).toEqual(ok({ draft }));
    expect(
      (await store.get<Session>(keys.session(owner)))!.value.draft,
    ).toEqual(draft);
    expect(
      await service.saveDraft(owner, { ...draft, productId: "none" }),
    ).toMatchObject({ ok: false });
    expect(await service.saveDraft("missing", draft)).toMatchObject({
      ok: false,
    });
  ...
  it("rejects altered totals, missing sessions and duplicate keys with another body", async () => {
    const { service, owner, store } = await setup();
    expect(
      await service.create(owner, randomUUID(), {
        ...input,
        expectedTotalInCents: 1,
      }),
    ).toMatchObject({ ok: false, error: { code: "PRICE_CHANGED" } });
    expect(await service.create("missing", randomUUID(), input)).toMatchObject({
      ok: false,
    });
    expect(
      await service.create(owner, randomUUID(), {
    ...
    const created = value(await service.create(owner, key, input));
    const replay = value(await service.create(owner, key, input));
    expect(replay.created).toBe(false);
    expect(replay.transaction.id).toBe(created.transaction.id);
    expect(
      await service.create(owner, key, {
        ...input,
        customer: { ...input.customer, fullName: "Different" },
      }),
    ...
    ).toMatchObject({ stockAvailable: 11, stockReserved: 1, stockOnHand: 12 });
  });
  it("reserves the final unit once across concurrent buyers and idempotent retries", async () => {
    ...
    await store.commit([
      {
        ...
        value: { ...p.value, stockAvailable: 1, stockOnHand: 1 },
      },
    ]);
    const other = (await service.bootstrap()).session.id;
    const results = await Promise.all([
      service.create(owner, randomUUID(), input),
      service.create(other, randomUUID(), input),
    ]);
    ...
    const [a, b] = await Promise.all([
      fresh.service.create(fresh.owner, key, input),
      fresh.service.create(fresh.owner, key, input),
    ]);
    expect(value(a).transaction.id).toBe(value(b).transaction.id);
  });
  it.each(["APPROVED", "DECLINED", "ERROR", "VOIDED"] as const)(
    "applies %s atomically once and controls reads by owner",
    async (status) => {
      const { service, owner, store, gateway } = await setup();
      gateway.status = status;
      const tx = value(
        await service.create(owner, randomUUID(), input),
      ).transaction;
      const [first, repeated] = await Promise.all([
        service.pay(owner, tx.id, payment),
        service.pay(owner, tx.id, payment),
      ]);
      ...
      expect(p).toMatchObject({
        stockOnHand: status === "APPROVED" ? 11 : 12,
        stockAvailable: status === "APPROVED" ? 11 : 12,
        stockReserved: 0,
      });
      const raw = (await store.get<Transaction>(keys.transaction(tx.id)))!
        .value;
      expect(
        value(await service.customer(owner, raw.customerId)),
      ).toMatchObject({ email: input.customer.email });
      expect(await service.customer("stranger", raw.customerId)).toMatchObject({
        ok: false,
      });
      expect(await service.transaction("stranger", tx.id)).toMatchObject({
        ok: false,
      });
      expect(await service.pay("stranger", tx.id, payment)).toMatchObject({
        ok: false,
      });
      if (status === "APPROVED") {
        expect(
          value(await service.delivery(owner, raw.deliveryId!)),
        ).toMatchObject({ status: "READY" });
        expect(
          await service.delivery("stranger", raw.deliveryId!),
        ).toMatchObject({ ok: false });
  ...
  it("holds uncertain submissions without releasing stock or charging again", async () => {
    ...
    expect(value(await service.pay(owner, tx.id, payment))).toMatchObject({
      status: "PENDING",
    ...
    await service.pay(owner, tx.id, payment);
    expect(gateway.create).toHaveBeenCalledTimes(1);
    expect(await service.clearDraft(owner)).toMatchObject({
      ok: false,
      error: { code: "PAYMENT_IN_PROGRESS" },
    });
    expect(
  ...
  it("reconciles a pending provider result once, tolerates unavailable and mismatched responses", async () => {
    ...
    gateway.get.mockResolvedValueOnce(
      ok({ ...gateway.provider(gateway.last!), reference: "OTHER" }),
    );
    expect(value(await service.transaction(owner, tx.id)).status).toBe(
      "PENDING",
    );
    ...
    const [a, b] = await Promise.all([
      service.transaction(owner, tx.id),
      service.transaction(owner, tx.id),
    ]);
  ...
  it("does not submit when unconfigured, releases definitive rejection and rejects mismatched amounts", async () => {
    ...
    expect(await service.pay(owner, tx.id, payment)).toMatchObject({
      ok: false,
    ...
    gateway.create.mockImplementationOnce(async (raw) =>
      ok({ ...gateway.provider(raw), amountInCents: 1 }),
    );
    expect(
      value(await service.pay(owner, tx2.id, payment)).submissionStatus,
    ).toBe("UNKNOWN");
  });
  it.each(["transaction", "product", "quote", "replay", "pay"] as const)(
    "expires an unsubmitted reservation through %s",
    async (route) => {
      const { service, owner, advance, store, gateway } = await setup(),
        key = randomUUID();
      const tx = value(await service.create(owner, key, input)).transaction;
      advance(900001);
      ...
      if (route === "replay")
        expect(
          value(await service.create(owner, key, input)).transaction.canPay,
        ).toBe(false);
      if (route === "pay")
        expect(value(await service.pay(owner, tx.id, payment)).canPay).toBe(
          false,
        );
      expect(
        (await store.get<Transaction>(keys.transaction(tx.id)))!.value.status,
      ).toBe("ERROR");
      expect(
        (await store.get<Product>(keys.product(input.productId)))!.value
          .stockAvailable,
      ...
      expect(gateway.create).not.toHaveBeenCalled();
      expect(
        value(await service.create(owner, randomUUID(), input)).created,
      ).toBe(true);
    },
  );
  it("cancels safely and prevents a stale expiry observation from releasing a claimed payment", async () => {
    ...
    expect(value(await service.transaction(owner, first.id)).status).toBe(
      "ERROR",
    );
    const second = value(
      await service.create(owner, randomUUID(), input),
    ...
    await store.commit([
      {
        key: keys.transaction(second.id),
        expectedVersion: stale.version,
        value: { ...stale.value, submissionStatus: "CLAIMED" },
      },
    ]);
    jest.spyOn(store, "pending").mockResolvedValue([stale]);
    advance(900001);
    await service.products();
    expect(value(await service.transaction(owner, second.id))).toMatchObject({
      status: "PENDING",
  ...
  it("reports contention after bounded retries and propagates unexpected storage failure", async () => {
    ...
    const commit = jest
      .spyOn(store, "commit")
      .mockRejectedValue(new WriteConflict());
    expect(await service.create(owner, randomUUID(), input)).toMatchObject({
      ok: false,
