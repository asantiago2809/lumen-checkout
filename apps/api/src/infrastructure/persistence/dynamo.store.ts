...
export class DynamoStore implements CheckoutStore {
    ...
    do {
      const response = await this.client.send(
        new QueryCommand({
          ...
          ExpressionAttributeValues: { ":pk": pending ? "PENDING" : "CATALOG" },
  ...
  products() {
    return this.query<Product>(false);
  }
  pending() {
      ...
      const condition =
        write.expectedVersion === null
          ...
          : {
              ...
              ExpressionAttributeValues: { ":version": write.expectedVersion },
            };
      const common = { TableName: this.table, ...condition };
      if (write.value === null)
        return { Delete: { ...common, Key: dynamoKey(write.key) } };
      ...
      return {
        Put: {
          ...
          Item: {
            ...
            ...(write.key.startsWith("TX#") && tx.status === "PENDING"
              ? { gsi1pk: "PENDING", gsi1sk: tx.createdAt }
              : {}),
            ...(write.key.startsWith("SESSION#")
