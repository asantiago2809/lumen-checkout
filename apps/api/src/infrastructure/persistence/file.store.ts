...
export class FileStore implements CheckoutStore {
  ...
  async products(): Promise<Stored<Product>[]> {
    ...
    return Object.entries(await this.read())
      ...
      .map(([, item]) => item as Stored<Product>);
  }
  async pending(): Promise<Stored<Transaction>[]> {
    ...
    return Object.entries(await this.read())
      .filter(
        ([key, item]) =>
          key.startsWith("TX#") &&
          (item.value as Transaction).status === "PENDING",
      )
      .map(([, item]) => item as Stored<Transaction>);
  }
  async commit(writes: Write[]): Promise<void> {
    const operation = this.tail.then(async () => {
      const state = await this.read();
      for (const write of writes)
        if ((state[write.key]?.version ?? null) !== write.expectedVersion)
