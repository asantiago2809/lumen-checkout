...
| Versión | E4 en implementación publicada eda790c; E5 tras ajuste final de labels/CSS. HEAD de cierre documental f84fc56, sin cambios de UI posteriores reportados. |
| URL/build | Vite5174 + Nest3002 reales; FileStore aislado; gateway/tokenización externos controlados |
...
A=APROBADO en escenario E4/E5 indicado; P=no ejecutado o parcial. Una aprobación de estado acredita comportamiento/visibilidad y comprobaciones del test; no implica revisión exhaustiva de cada píxel. 390x844 y1024x768 no fueron ejecutados. La prioridad375x667 y el paisaje 667x375 quedan explícitos.

| Pantalla/estado | 320×568 | 375×667 | 390×844 | 667×375 | 768×1024 | 1024×768 | 1440×900 |
|---|---|---|---|---|---|---|---|
| Producto disponible | A E5 | A E4/E5 | P | A E5 | A E5 | P | A E4/E5 |
| Producto cargando/error/agotado | P | P | P | P | P | P | P |
| Tarjeta y entrega vacío | P | A E4 | P | P | P | P | A E4 |
| Formulario con errores | A E5 | A E4/E5 | P | A E5 | P | P | A E4/E5 |
| Resumen normal | P | A E4 | P | P | P | P | A E4 |
...
Paisaje: catálogo, formulario con errores y resumen largo se probaron a 667x375 con scroll/CTA accesibles. El resto de los estados en paisaje quedan P; no se declara una matriz total. Los estados loading/agotado tienen Jest pero no pase visual independiente archivado.
...
| Layout producto/formulario-error/resumen largo375x667 | APROBADO | APROBADO | APROBADO | E5 cambia viewport en todos los proyectos |
| Tab y Escape, foco inicial/trap/restaurado | APROBADO | APROBADO | APROBADO |24 Tabs permanecen en diálogo; Escape devuelve foco |
| Shift+Tab/Enter en todas las etapas | PENDIENTE | PENDIENTE | PENDIENTE | No ejecutado de forma independiente |
| Reflow320 px sin overflow en layouts indicados | APROBADO | APROBADO | APROBADO | E5; no equivale a zoom 200% |
| Zoom200% | PENDIENTE | PENDIENTE | PENDIENTE | No ejecutado |
...
| Reduced motion dinámico | PENDIENTE | PENDIENTE | PENDIENTE | CSS definido, emulación no ejecutada |
...
| VIS-05 | CTA alcanzable, área táctil ≥44×44 px; controles/CTA alto ≥48 px; texto de inputs ≥16 px | PENDIENTE | CTA catálogo >=44 px medido; CTA modal/resumen alcanzable. Falta medir todos los targets/input fonts y mínimo48 px por control. |
...
| VIS-12 | Titulares mantienen espacios entre palabras cuando el breakpoint oculta saltos `<br>`; reduced motion elimina movimiento de flecha del CTA | PENDIENTE | Espacios corregidos y heading/retest visual PASS; reduced-motion requiere prueba dinámica. |
...
| UX-07 | Editar datos conserva progreso permitido y jamás muestra PAN/CVC en resumen | PENDIENTE | Resumen sin PAN/CVC y refresh seguros comprobados; recorrido específico editar/regresar completo no registrado. |
| UX-08 | Total largo/nombre/dirección/referencia largos envuelven sin recorte | PENDIENTE | Nombre/dirección/complemento largos pasan en 4 tamaños. Referencia64 sin espacios/total 9 cifras no ejecutados en navegador. |
| UX-09 | Cambio de total exige revisión antes de confirmar; no pago silencioso de importe diferente | APROBADO | E4 QA-M02: cambio de precio exige revisar y produce0 cargos/0 transacciones. |
...
| UX-14 | Salir de vista durante transacción no comunica cancelación inexistente | PENDIENTE | Recarga probada; falta recorrido explícito de salida/cierre durante transacción en progreso. |
...
| A11Y-01 | `lang`, título de documento, un h1, landmarks y enlace saltar al contenido correctos | PENDIENTE | Axe producto pasa, h1 comprobado; enlace saltar/landmarks completo no registrado por auditor. |
| A11Y-02 | Un solo diálogo activo, nombre accesible, `aria-modal`, fondo inerte y focus trap correcto | PENDIENTE | Nombre/aria-modal y trap Tab comprobados; aislamiento inerte del fondo no probado de forma separada. |
...
| A11Y-07 | Reduced motion elimina animación no esencial; ninguna información depende de ella | PENDIENTE | CSS existe; emulación prefers-reduced-motion no ejecutada. |
...
| PRIV-02 | Evidencia y documentación pública sin PII real ni credenciales | APROBADO | Datos ficticios, escáner110 archivos PASS, evidencia sin PAN/CVC ni credenciales. |
...
**Diseño local aprobado en el alcance documentado; revisión ampliada pendiente en los controles señalados.** Se verificaron jerarquía moderna, identidad original, estados financieros honestos, recuperación segura, flujo móvil/escritorio y layout estrecho/paisaje de los formularios principales. Falta dispositivo/teclado/lector de pantalla real y los escenarios adicionales explícitos.

La entrega completa continúa bloqueada por configuración API pública y sandbox real, como registra [auditoría 82](final-audit.md). Esta revisión no certifica cobros externos ni disponibilidad cloud.
