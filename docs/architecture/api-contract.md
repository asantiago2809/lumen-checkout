...
La interfaz distingue cambios pendientes, guardado en curso, datos confirmados y fallo con reintento. Tanto continuar como cerrar esperan la confirmación; si falla el cierre, conserva el formulario. Una respuesta antigua no marca como guardadas las ediciones más recientes. La recarga durante los 500ms anteriores al autoguardado o mientras la petición sigue pendiente puede perder la última edición todavía no confirmada; el aviso pide esperar. La tarjeta y los consentimientos nunca se recuperan desde el borrador.
...
**Extensión opcional, NO implementada:** `POST /webhooks/payment` no es una ruta disponible en esta versión. Un futuro endpoint para `transaction.updated` deberá comprobar firma de eventos, ambiente sandbox, referencia, importe y moneda antes de aplicar; procesamiento idempotente, `200` para evento válido repetido, `400/401` inválido. Configurar el webhook del comercio compartido requiere analizar su impacto. La implementación actual reconcilia mediante consultas autenticadas al backend. Un worker programado de reconciliación también es opcional y **no está implementado**; podría reutilizar el mismo caso de uso sin acoplarlo a HTTP.
